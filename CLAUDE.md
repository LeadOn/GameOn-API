...
- **Couche External** : `GameOn.External/Llm/` avec `ILlmService` (seam volontairement agnostique du provider : `ModelName`, `IsConfigured`, `GenerateJsonAsync(system, user, jsonSchema)`) et `GeminiLlmService`. Variables d'env : `GEMINI_API_KEY`, `GEMINI_MODEL` (défaut `gemini-3.6-flash`), `LLM_TIMEOUT_SECONDS` (défaut 180). Basculer vers un llama.cpp auto-hébergé plus tard = une nouvelle implémentation de `ILlmService`, rien d'autre à toucher.
...
- **Génération à la demande, jamais automatique** (décision utilisateur explicite du 2026-09-17, après un premier jet asynchrone) : aucun rapport n'est écrit sans que quelqu'un l'ait demandé. Le `BackgroundService`, la mise en file par scan, la machinerie de statuts/tentatives et les commandes associées ont été **supprimés**. Un `POST` génère et renvoie l'analyse **dans la même requête** (~15 s), le front met un spinner.
...
- **Garde-fou de débit** (2026-09-17) : `GeminiLlmService` sérialise les appels au modèle via un `SemaphoreSlim(1)` **statique**, donc process-wide malgré le service enregistré en `Scoped`. Rien d'autre dans le projet ne limite le débit, et l'allocation du free tier est à la minute : une génération tenant ~15 s, les sérialiser plafonne mécaniquement à ~4 req/min, très en dessous de n'importe quelle limite publiée, sans avoir à tenir une fenêtre glissante. L'attente du créneau est **bornée à 30 s** (`QueueWait`), volontairement plus courte que le timeout de 60 s du front sur cet endpoint : une requête qui mourrait de toute façon dans le navigateur doit être refusée ici, avec un statut exploitable. Le chemin « rapport déjà en cache » ne passe pas par le sémaphore, puisqu'il n'appelle pas le modèle.
...
- **Routes** : `GET lol/Coach/{matchId}/player/{playerId}` — libre, **ne déclenche jamais rien**, renvoie 404 tant que personne n'a demandé l'analyse (le front doit traiter ce 404 comme « propose le bouton », pas comme une erreur). `POST lol/Coach/{matchId}/player/{playerId}` — `[Authorize]`, n'importe quel joueur connecté sur n'importe quelle partie, génère ou renvoie le rapport déjà stocké. Le paramètre `force=true` (régénération) n'est honoré que si l'appelant a le rôle `gameon_admin` : n'importe qui peut le passer, personne d'autre ne l'obtient.
- **Le coach s'appelle « Raimmus » (Rammus + AI) et chambre les mauvaises parties** — `LoLCoachPrompt.SystemPrompt`, **version 2** (2026-09-17). Le ton est calibré sur la performance et sur rien d'autre : chambrage franc sur une partie ratée, factuel et sec sur une partie correcte, reconnaissance sobre sur une bonne partie. Vérifié en direct : Ahri 18/11/22 → note 5,5 et ton mesuré ; Nautilus 1/9/8 → note 1,5 et « tu as servi de buffet à volonté pour Thresh et Lee Sin ».
  ...
  - ⚠️ Les rapports déjà en base ont `PromptVersion = 1` et restent servis tels quels (ton neutre). Pour les repasser en v2 : `POST lol/Coach/{matchId}/player/{playerId}?force=true` avec le rôle `gameon_admin`.
...
- **Reste à faire** : appliquer la migration `20260917122843_Added_LoLGameCoachReport` en prod, poser `GEMINI_API_KEY` et `GEMINI_MODEL` dans l'environnement du conteneur, puis brancher le front (bouton « Analyser cette partie » + spinner).
