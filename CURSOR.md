...
LoLQueue — types de queue Riot normalisés (Id = queueId Riot, clé naturelle), synchronisés quotidiennement depuis `queues.json` ; `LoLGame.QueueId` la référence en FK nullable (coexiste avec l'ancien `LoLGame.QueueType` en string)
Commun
...
Reste à faire (pas bloquant, à la main par l'utilisateur quand il le souhaite) : rattrapage de masse des `LoLGame.QueueId` pour les parties déjà en base avant l'introduction de `LoLQueue` (seules les parties synchronisées/mises à jour depuis le 2026-07-16 ont leur `QueueId` rempli automatiquement par `UpdateLoLGameCommandHandler`).
