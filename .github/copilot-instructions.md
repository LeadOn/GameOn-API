...
LoLQueue — types de queue Riot normalisés (Id = queueId Riot, clé naturelle), synchronisés quotidiennement depuis `queues.json` ; `LoLGame.QueueId` la référence en FK nullable. `LoLGame.QueueType` (string) est conservé en lecture seule pour compatibilité API, dérivé de `LoLQueue.Description`/`Map` (plus de dictionnaire en dur) — à supprimer une fois le front basculé sur `QueueId`/les routes `/lol/Queue`
Commun
...
Reste à faire (pas bloquant) : une fois le front consomme `QueueId`/`LoLQueue` (routes `/lol/Queue` et `/lol/Queue/player/{playerId}`) plutôt que `LoLGame.QueueType`, supprimer complètement cette colonne/propriété (migration EF Core de suppression + retrait de la propriété du domaine).
